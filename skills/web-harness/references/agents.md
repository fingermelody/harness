...
本文档定义了 WebHarness 框架中的 **7 个核心 Agent 角色**。每个角色都有明确的职责边界、技能矩阵和协作契约。
...
3. **Coder → Reviewer**：提交代码供 Review
4. **Reviewer → Coder**：Review 意见
5. **Reviewer → Deployer**：Review 通过后触发部署
6. **Deployer → Tester**：部署完成通知测试
7. **Tester → Evaluator**：测试报告
8. **Evaluator → Deployer**：质量门禁判定
9. **Deployer → PM**：发布状态 + 线上指标
10. **全员 → PM**：进度反馈
